...
function CategoryBlogTr({ params }) {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const blogCounterRes = await fetch(`http://localhost:8080/category/getBlogCounter?categorySlug=${params.categories}&lang=EN`)
            ...
            try {
                const postsRes = await fetch(`http://localhost:8080/category/${params.categories}/posts?lang=EN`)
    ...
    const loadMore = () => {
        ...
        fetch(`http://localhost:8080/category/${params.categories}/posts?lang=EN&page=${currentPage}`).then(res => res.json()).then(data => {
